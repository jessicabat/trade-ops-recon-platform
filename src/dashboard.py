...
st.set_page_config(
    page_title="Trading Ops Dashboard",
    page_icon="📊",
    layout="wide",
...
# --- PICK DATE IN CODE (LATEST AVAILABLE) ---
with engine.connect() as conn:
    dates = conn.execute(
        text("SELECT DISTINCT trade_date FROM internal_trades ORDER BY trade_date DESC")
    ).fetchall()
    date_options = [d[0] for d in dates]

if not date_options:
    st.error("No data available in internal_trades.")
    st.stop()

selected_date = date_options[0]  # latest date
...
with engine.connect() as conn:
    trade_count = conn.execute(
        text("SELECT COUNT(*) FROM internal_trades WHERE trade_date = :d"),
        {"d": selected_date},
    ).scalar()
    break_count = conn.execute(
        text("SELECT COUNT(*) FROM recon_trades WHERE recon_date = :d"),
        {"d": selected_date},
    ).scalar()
    pos_breaks = conn.execute(
        text("SELECT COUNT(*) FROM recon_positions WHERE recon_date = :d"),
        {"d": selected_date},
    ).scalar()
    total_pnl = conn.execute(
        text("SELECT SUM(total_pnl) FROM daily_pnl WHERE pnl_date = :d"),
        {"d": selected_date},
    ).scalar() or 0
...
col2.metric(
    "Trade Breaks",
    break_count,
    delta=f"{(break_count/trade_count*100):.2f}%" if trade_count > 0 else "0%",
)
